...
class Solution:
    def minimumFinishTime(
        ...
        dp[1] = changeTime + min(tires, key=lambda i: i[0])[0]
        for f, r in tires:
            idx, y = 1, f + changeTime
            t = f
            while idx < numLaps:
                t *= r
                y += t
                if t > dp[1] and (y / idx) > dp[1]:
                    break
                idx += 1
                dp[idx] = min(dp[idx], y / idx)

        res = -changeTime
        q = []
        # print(dp)
        dk = [float("inf")] * (numLaps + 1)
        for ii, jj in enumerate(dp):
            if jj != float("inf"):
                heapq.heappush(q, (jj, ii))
        dk[1] = dp[1]
        dk[0] = 0
        for ii in range(1, numLaps + 1):
            for jj, kk in q:
                if ii - kk >= 0:
                    dk[ii] = min(dk[ii], dk[ii - kk] + jj * kk)
        # print(dk)
        return int(dk[-1] - changeTime)
