# -*- coding: utf-8 -*-
# @Author: gunjianpan
# @Date:   2021-06-20 11:40:51
# @Last Modified by:   gunjianpan
# @Last Modified time: 2021-06-20 11:41:04

"""
5790. 查询差绝对值的最小值 显示英文描述 
通过的用户数98
尝试过的用户数137
用户总通过次数101
用户总提交次数184
...
class Solution:
    def minDifference(self, nums: List[int], queries: List[List[int]]) -> List[int]:
        ...
        for ii, jj in queries:
            tmp = cs[jj + 1].copy()
            tt = []
            for idx, k in enumerate(cs[ii]):
                tmp[idx] -= k
                if tmp[idx] != 0:
                    tt.append(idx)
            tmp = sorted(tt)
            t = [tmp[ii + 1] - tmp[ii] for ii in range(len(tmp) - 1)]
            res.append(min(t) if t else -1)
        return res
