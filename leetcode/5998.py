...
class Solution:
    def maximumEvenSplit(self, finalSum: int) -> List[int]:
        if finalSum >> 1 << 1 != finalSum:
            return []
        if finalSum <= 4:
            return [finalSum]

        left, right = 2, 1 << (int(math.log2(finalSum)))
        # print(left, right)
        while left < right:
            mid = (left + right) // 2
            mid = mid >> 1 << 1
            if finalSum - (mid + 2) * mid // 4 - mid <= 0:
                right = mid
            else:
        ...
        if finalSum - (left + 2) * left // 4 == 0:
            return list(range(2, left + 2, 2))
        return list(range(2, left + 2, 2)) + [finalSum - (left + 2) * left // 4]
