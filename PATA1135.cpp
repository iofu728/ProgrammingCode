...
void BFSlayer(node* root){
	...
	while(!q.empty()){
		...
		if(front->lchild!=NULL){
			if(front->lchild->vis==true) front->lchild->num=front->num+1;
			else front->lchild->num=front->num;
			front->lchild->layer=front->layer+1;
			q.push(front->lchild);
		}
		if(front->rchild!=NULL){
			if(front->rchild->vis==true) front->rchild->num=front->num+1;
			else front->rchild->num=front->num;
...
bool BFSys(node* root){
	...
	while(!q.empty()){
		...
		if(layervis.size()<front->layer) {
			...
			if(front->layer>=2&&layervis[front->layer-2]==false&&layervis[front->layer-1]==false) return false;
		}
		else{
			if(layervis[front->layer-1]!=front->vis) return false;
		}
		if(front->lchild==NULL&&front->rchild==NULL){
			if(num==0) num=front->num;
			else {
